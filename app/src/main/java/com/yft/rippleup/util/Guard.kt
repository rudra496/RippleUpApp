...
/**
 * Client-side guard rails for action logging (ported from the proven RippleUp infra):
 ...
 *
 * HONEST LIMIT: client-side only; a determined attacker with a decompiled APK can
 * bypass it. True anti-cheat needs a server. This raises the bar far above
 * "open the DB and type any number".
 */
object Guard {
    ...
    private val secret: ByteArray by lazy {
        val seed = "ripplup::" + runCatching { Build.FINGERPRINT.reversed() }.getOrDefault("demo") + "::" + longSeed()
        seed.toByteArray()
    }
